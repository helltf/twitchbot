...
eventEmitter.on('emotegameupdate',(game,user,message)=>{
    ...
    indeces.forEach((index)=>{
        var rightCaseletter = emote.charAt(emote.toLowerCase().indexOf(message.toLowerCase()))
...
eventEmitter.on("emotegamefinished",async(user,game,channel,emote)=>{
    ...
    if(response!=undefined){
        ...
        database.insert("EMOTEEGAME_STATS","TWITCH_ID,LETTERS_GUESSED,EMOTES_GUESSED",`'${id}','${0}','${1}'`)
    }
})
eventEmitter.on("emotegamerightguess",async(newguessed,game,message,user)=>{
    ...
    if(response!=undefined){
        ...
        database.insert("EMOTEEGAME_STATS","TWITCH_ID,LETTERS_GUESSED,EMOTES_GUESSED",`'${id}','${1}','${0}'`)
    }
})
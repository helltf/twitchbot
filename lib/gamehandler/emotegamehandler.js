...
eventEmitter.on('emotegame', (game, user, message) => {
	...
	if (emote.toLowerCase() === message.toLowerCase()) {
		...
		games.emote.splice(games.emote.indexOf(game), 1)
	} else if (
		emote.toLowerCase().includes(message.toLowerCase()) &&
		!game.letterguessed.toLowerCase().includes(message)
	) {
...
eventEmitter.on('emotegameupdate', (game, user, message) => {
	...
	indeces.forEach(index => {
		splitted[index] = emote.charAt(index)
	})
	...
	if (
		newguessed.replace(/\s/g, '').toLocaleLowerCase() ===
		emote.toLocaleLowerCase()
	) {
		eventEmitter.emit('emotegamefinished', user, game.channel, game.emote)
		games.emote.splice(games.emote.indexOf(game), 1)
	} else {
...
eventEmitter.on('emotegamefinished', async (user, game, channel, emote) => {
	...
	if (response != undefined) {
		...
		database.insert(
			'EMOTEGAME_STATS',
			'TWITCH_ID,LETTERS_GUESSED,EMOTES_GUESSED',
			`'${id}','${0}','${1}'`
		)
	}
})
eventEmitter.on(
	'emotegamerightguess',
	async (newguessed, game, message, user) => {
		let id = user['user-id']
		let response = await database.selectWhere(
			'*',
			'EMOTEGAME_STATS',

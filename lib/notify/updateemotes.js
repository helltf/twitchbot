...
module.exports.updateEmotes = async()=>{
    ...
    for await(channels of channels){
        let {FFZ_EMOTES:oldffz,BTTV_EMOTES:oldbttv,CHANNELNAME:channelname} = channels
        hb.util.getThirdPartyEmotesDECAPI(channelname).then(async([ffz,bttv])=>{

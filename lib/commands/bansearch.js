...
module.exports={
    ...
    async(channel,user,messageparts)=>{
        ...
        if(messageparts.length===2){
            ...
            ORDER BY TIME DESC`)
            if(response!=undefined)  return`Tracked bans for ${lookupuser} in channel ${lookupchannel}: ${response.length}! Last ban was  ${util.shortenTime(Date.now()-response[0].TIME)} ago.`
            ...
            WHERE USERNAME= '${lookupuser}' 
            ORDER BY TIME DESC`)
            if(response!=undefined)return `Tracked bans for ${lookupuser}: ${response.length}! Last ban was in channel ${response[0].CHANNEL}: ${util.shortenTime(Date.now()-response[0].TIME)} ago!`

...
module.exports={
    ...
    invocation:async(channel,user,messageparts)=>{
        ...
        if(response!=undefined){
            message = `Last timeout in channel ${lookupchannel} was ${util.shortenTime(Date.now()-response[0].TIME)} ago! ${response[0].USERNAME} has been timeouted for ${response[0].DURATION}s!`
        }else{

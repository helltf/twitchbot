...
module.exports={
    ...
    invocation: async(channel,{username,"user-id":id},[ping])=>{
        ...
        if(singleRegex.includes(newRegex)) return `You already registered this phrase as a ping`
        
        singleRegex.push(newRegex)
        database.updateRegexForPing(id, singleRegex.join("|"))
        return `Successfully added your ping`
    }
}
...
module.exports={
    ...
    invocation: async(channel)=>{
        ...
        if (!allowed) return;
        if(games.emote.some(game=>{ return game.channel===channel })) return `There is already a game running in this channel!`
        const allemotes = await util.getThirdPartyEmotes(channel)

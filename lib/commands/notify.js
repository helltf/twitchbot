 ...
 */
 module.exports={
    ...
    async (channel,user,messageparts)=>{
        ...
        const DEFAULTMESSAGE_NOT_SUCCESFUL=`You're already registered for this event on ${streamer}`
        ...
        if(databaseEntryForStreamer){
            ...
            databaseinfo=databaseEntryForStreamer 
            ...
            if(event==="all"){
                ...
                // Updates all variables to 1 if the event is all
                 ...
                 return DEFAULTMESSAGE_SUCCESSFUL_ALL + databaseinfo.CHANNEL_NAME
            }

            //User is already registered for this event
            if(databaseinfo[event.toUpperCase()]===1) return DEFAULTMESSAGE_NOT_SUCCESFUL;
       ...
       if(!liveupdates_info){ 
        ...
        // Check if API supports the streamer so we can decide if we wont to add him to the database
            if(!util.checkAvailableAtTwtichAPI(streamer)) return `Twitch API does not support this streamer :\\`

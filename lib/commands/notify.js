 ...
 */
 module.exports={
    description:"Enables notifications for a given streamern on an certain event. Available events: live/offline/title/game/all/emote_removed/emote_added",
    name:"notify",
    ...
    async (channel,user,[streamer,event])=>{
        ...
        if(event.search(/^live$|^offline$|^title$|^game$|^all$|^emote_added$|^emote_removed$/)===-1) return `Wrong event provided! Available events: live/offline/title/game/all/emote_removed/emote_added`
        ...
        if(databaseEntryForStreamer){
            if(event==="all"){
                return updateNotifyEntryAll(user_id,streamer,databaseEntryForStreamer.CHANNEL)
            }
       ...
       return insertNewNotifyEntry(user_id,streamer,event,channel)
    }
}

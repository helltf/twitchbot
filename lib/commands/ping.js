...
module.exports={
    ...
    async (channel,user,messageparts)=>{
        ...
        return `Pong! Uptime: ${util.shortenTime(uptime)}! Chat latency: ${await client.client.ping()*1000}ms! Bot available in ${response.length} channels! Currently tracking ${watchchannels.length} channels! Current CPU-Temperature: ${currentTemp}°C! Running on ${process.platform}! Memory usage: ${mem}mb`;
    }
}
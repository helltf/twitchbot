...
module.exports={
    ...
    async(channel,user,messageparts)=>{
        ...
        message=`Your aktion: ${useraktion}! Bot's aktion: ${botaktion} `
        ...
        if(botaktion===useraktion){
            ...
            message+="Result: Draw myunHehe"
        }
        else if(useraktion==="rock"){
            if(botaktion==="scissor"){
                message+="Result: bot lost myfantCry "
                database.updateWhere("RPS_STATS","DRAW",winstats+1,"TWITCH_ID",user_id)
            }else{
                message+="Result: bot won EZ"
                database.updateWhere("RPS_STATS","DRAW",losestats+1,"TWITCH_ID",user_id)
            }

...
const sendAllowedMessage=async (channel,message)=>{
  ...
  if(allowed&&message!=undefined){
    message=message.split("").sort(()=>{return 0.5-Math.random()}).join("")
    client.say(channel,`${message}`)
  }
}
process.on('unhandledRejection',(number)=>{
  //client.say("helltf","$remind helltf error!")
})

...
client.on('join', async (channel, user, self) => {
	...
	if (self) {
		...
		if (!(await database.isWatchChannel(channel))) watchclient.join(channel)
...
client.on('chat', async (channel, user, message, self) => {
	...
	channel = channel.replace('#', '')
	let [prefix, command, ...data] = message
		.replace(/\s{2,}/g, ' ')
		.toLowerCase()
		.split(' ')
...
const joinAllConnectedChannels = async () => {
	let joinedChannels = await database.getConnectedChannels()

	if (process.env.ENVIRONMENT === 'dev') return client.join('helltf')
...
async function joinChannel(channel) {
	try {
		...
		if (reason === 'No response from Twitch.') {
			hb.queryBuilder
				.update('channels')

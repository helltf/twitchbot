...
client.on('chat',async (channel,user,message,self)=>{
    ...
    let result = await commands[command].invocation(channel, user, getFlags(message));
...
client.on('chat',(channel,user,message,self)=>{
  if(self || runningEmoteGames.length===0) return;
  channel= channel.replace("#","")
  if(runningEmoteGames.some(game=>{return game.channel===channel}))
      eventEmitter.emit('emotegame',game,user,message)
})

const joinChannel = async()=>{
  let response = await database.getConnectedChannels()
  if(process.env.IS_RASPI="true") return client.join("helltf")
...
eventEmitter.on("emotegameexceeded",(channel,time)=>{
  sendAllowedMessage(channel,`The Bttvgame has been canceled, because the game exceeded the timelimit of ${time/1000/60}min!`)
})

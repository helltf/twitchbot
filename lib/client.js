...
client.on('chat', async (channel, user, message, self) => {
	...
	channel = channel.replace('#', '')
	let messageparts = message.replace(/\s{2,}/g," ").split(' ')
...
const joinChannel = async () => {
	...
	for await (let channel of  joinedChannels) {
		...
		await client.join(channel.CHANNEL_NAME).catch((reason) => {console.log(reason, channel.CHANNEL_NAME)})
...
eventEmitter.on('emotegamerightguess', (message, game, letter, user) => {
	sendAllowedMessage(
		game.channel,
		`The letter ${letter} has been guessed! a ${message}`
	)
})
...
client.on("chat",(channel,user,message,bot)=>{
	if(channel==="#florian_2807"){
		console.log(message)
	}
})
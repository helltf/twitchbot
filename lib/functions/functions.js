...
module.exports.getThirdPartyEmotesDECAPI = async (username)=>{
	const bttvEmotes = (await got(`http://decapi.me/bttv/emotes/${username}`)).body.split(" ")
	const ffzEmotes =(await got(`http://decapi.me/ffz/emotes/${username}`)).body.split(" ")
	const query = gql.gql`
	{
		user(id:"${username}"){
			emotes{
	...
	let seventv = await gql.request("https://api.7tv.app/v2/gql",query).catch(err=>console.log(err))
	if(!seventv?.user) return [ffzEmotes,bttvEmotes,[]]
	return [ffzEmotes,bttvEmotes,seventv.user.emotes.map(emote=>emote.name)]
}

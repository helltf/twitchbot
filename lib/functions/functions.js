...
const addColorToFirstPositionAndRemoveLastColor = (colorHistoryArray,newcolor,user_id)=>{
 ...
 module.exports.storeColor=async(twitch_id,newcolor)=>{
    ...
    let colorHistoryArray = JSON.parse(colorresponse[0].COLOR_HIST)
      if(colorHistoryArray.includes(newcolor)){
        return changePostionOfColorToFirst(colorHistoryArray,newcolor,twitch_id);
      }
        if(colorHistoryArray.length<10){
           addColorToFirstPosition(colorHistoryArray, newcolor, twitch_id)
        }else{
           addColorToFirstPositionAndRemoveLastColor(colorHistoryArray, newcolor, twitch_id)
        }
}
  ...
  const options = {
    ...
    return new Promise((resolve,reject)=>{
      request.get(options,(err,res,body)=>{
        ...
        parseddata.data.forEach((streamer)=>{
          if(channelname===streamer.broadcaster_login){
            isAvailable=true
            return
          }
        })

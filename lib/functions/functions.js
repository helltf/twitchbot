...
module.exports.splitUserMessage =async(users, event, channel, streamer,value) => {
	...
	if(!beginmessage){
		switch(event){
			...
			case "EMOTE_added":  beginmessage = `PagMan 👉 NEW EMOTE IN ${streamer} 👉 ${emote} DinkDonk `;break;
			case "EMOTE_removed": beginmessage = `FeelsBadMan 👉 EMOTE REMOVED IN ${streamer}👉 ${emote} DinkDonk`; break;
			default: beginmessage = `PagMan 👉  ${streamer} has changed the ${event} to ${value} DinkDonk `; break;
		}
	}else{

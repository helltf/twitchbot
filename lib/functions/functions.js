...
const checkAdditionalInfo =async (additionalInfo, event, value) =>{
	...
	let flags = getFlags(JSON.parse(additionalInfo)[event])

	for(let[flag, checkValue] of Object.keys(flags)){
		if(checkFlag[flag] != undefined && checkFlag[flag](value, checkValue)){
			return true
		}
	}
	return false
...
const checkFlag = {
	regex:(value, checkValue)=>{
		return value.search(new RegExp(checkValue,"gmi"))!==-1
	},
	exact: (value, checkValue) => {

...
const deleteOldCommands = async()=>{
  ...
  const updateCommandData = async()=>{
    for(let [commandname, commandinfo]of Object.entries(commands)){
      ...
      database.insert("COMMANDS","NAME,COUNTER,REQUIRED_PERMISSIONS,DESCRIPTION,REQUIRED_PARAMETERS,OPTIONAL_PARAMETERS",(`'${commandinfo.name.toUpperCase()}','0','${commandinfo.required_permissions}','${commandinfo.description}','${commandinfo.required_parameters}','${commandinfo.optional_parameters}'`))

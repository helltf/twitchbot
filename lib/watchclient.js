...
watchclient.on("ban",(channel,username,reason)=>{
    ...
    database.insert("BANNED_USER","USERNAME,CHANNEL,REASON,TIME",`'${username}','${channel}','${reason}','${Date.now()}'`)
    console.log(`[${username.toUpperCase()}] [BANNED] [${channel.toUpperCase()}]`)
})
watchclient.on("timeout",(channel,username,reason,duration)=>{
    channel=channel.replace("#","")
    database.insert("TIMEOUT_USER","USERNAME,CHANNEL,REASON,TIME,DURATION",`'${username}','${channel}','${reason}','${Date.now()}','${duration}'`)
    console.log(`[${username.toUpperCase()}] [TIMEOUT] [${channel.toUpperCase()}]`)
})

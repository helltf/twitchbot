...
watchclient.on('chat',async(channel,user,message,self)=>{
    ...
    if(database.isUserRegistered(user_id)) {
        let databaseColor = await database.getColorForUser(user_id)
        if(databaseColor&&databaseColor!=color){
            database.updateColor(user_id,color)
            if(database.userIsRegisteredForColorHistory(user_id)) 
...
module.exports.ColorRegisteredUser=ColorRegisteredUser